...
#                          self.layout.addWidget(widget.frame)
          ...
          #If we are a dict, create a vertical layout and add subwidgets
              ...
              if "template" in template:
                  for k,v in value.items():
                      self.widgets[k] = SmartWidget().init(k, v, template["template"], self.removeCallback )
              self.draw()
          else:
   ...
   def draw( self ):
      ...
      if self.type == "dict":
         ...
         print("Removing "+str(itemCount)+" items")

         button = QPushButton()
         name   = self.layout.itemAt(0).widget()
#         frame  = self.layout.itemAt(1).widget()
         button = self.layout.itemAt(itemCount - 1).widget()
...
#         self.layout = QVBoxLayout()
         ...
         for k,v in self.widgets.items():
             subLayout.addWidget( self.widgets[k].frame)
